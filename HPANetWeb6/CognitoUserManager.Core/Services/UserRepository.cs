...
namespace CognitoUserManager.Core.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        private readonly AssumeRoleService _assumeRoleService;
        private readonly HttpContext _httpContext;

        public UserRepository(IOptions<AppConfig> appConfig, UserContextManager userManager, AssumeRoleService assumeRoleService, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<UserSignUpResponse> CreateUserAsync(UserSignUpModel model)
        {
           ...
           // var credentials = await _assumeRoleService.GetAssumedRoleCredentialsAsync();

            // create the CognitoClient with the credentials
           // var cognitoClient = new AmazonCognitoIdentityProviderClient(credentials, RegionEndpoint.GetBySystemName(_cloudConfig.Region));
           ...
           // AdminCreateUserResponse userResponse = await cognitoClient.AdminCreateUserAsync(adminSignupRequest);
            ...
            //    UserPoolId = _cloudConfig.UserPoolId
            //};
            //AdminAddUserToGroupResponse grpResponse = await AdminAddUserToGroupAsync(addToGrpRequest);
            ...
            try
            {
                ...
                if (result.Item1.Username != null)
                {
                    await _userManager.SignIn(_httpContext, new Dictionary<string, string>() {
                        ...
                        {ClaimTypes.Role, result.Item1.ClientID}
                    });
                }
        ...
        public async Task<UserProfileResponse> GetUserAsync(string userId)
        {
            ...
            var userGroupAdmin = await _provider.AdminListGroupsForUserAsync(new AdminListGroupsForUserRequest
            {
                Limit = 2,
                Username = userId,
                UserPoolId = _cloudConfig.UserPoolId
            ...
            var listGroup = new List<string>();
             listGroup = userGroupAdmin.Groups.Select(x =>x.GroupName).ToList();
